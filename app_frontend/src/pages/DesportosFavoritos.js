...
export function DesportosFavoritos() {
    ...
    const getAllLocalidades = async (e) => {
        axios.get(`${API_URL}/users/allDesportos`)
                .then(response => {
                    setAllLocalidades(response.data)
                    setAllLocalidadesNames(response.data.map(({nomeDesporto})=> nomeDesporto))
                })
    ...
    const getLocalidadesFav = async (e) => {
        ...
        axios.get(`${API_URL}/users/desportosFav`, {headers: headers})
                .then(response => {
                    setLocalidadesFav(response.data.map(({nomeDesporto})=> nomeDesporto))
                })
                .catch(e => console.log(e))
    }
    ...
    const handleAddFavorito = async (e) => {
        ...
        if (localidadesFav.includes(newLocalidade.current.value)) {
            setAlert(true)
            setAlertMsg("Desporto Favorito já existente")
        } 
        else if (!allLocalidadesNames.includes(newLocalidade.current.value)) {
            setAlert(true)
            setAlertMsg("Desporto não existente")
        }
        else {
            const newLocalidadeID = allLocalidades.find(localidade => {
                return localidade.nomeDesporto === newLocalidade.current.value
            }).idDesporto

            const headers = { "authorization": "Bearer " + localStorage.getItem("token") }
        
            axios.post(`${API_URL}/users/addFavorito?desporto=${newLocalidadeID}`, null, {headers: headers})
                    .then(response => {
    ...
    const handleRemoveFavorito = async (e) => {
        ...
        const deleteLocalidadeID = allLocalidades.find(localidade => {
            return localidade.nomeDesporto === deleteLocalidade
        }).idDesporto

        const headers = { "authorization": "Bearer " + localStorage.getItem("token") }
        
        axios.delete(`${API_URL}/users/removeFavorito?desporto=${deleteLocalidadeID}`, {headers: headers})
                .then(response => {
                    console.log(response)
                    setChanges(changes + 1)
    ...
    return(
        <>
            ...
            <section className="login-info-form w3-mobile">
                <h1 style={{marginBottom: "40px"}}>Desportos Favoritos</h1>
                ...
                <div className="paragensFavoritas">
                    {
                        ...
                        (
                          <div className="containerDiv">
                            ...
                            <h2>Não possui Desportos Favoritos</h2>
                        )
                    }
                    
                </div>

                <div style={{marginTop: "20px", cursor: "pointer"}} onClick={handleTogglePopupNewLocalidade} className="registerLink">Adicionar novo Desporto Favorito</div>
                ...
                <div className={`popup ${popUpNewLocalidade ? 'active' : ''}`}>
                    <div className="overlay">
                        <form className="overlayContent">
                            <label>Novo Desporto Favorito</label>
                            <p style={{color: "red", marginBottom:"10px", marginTop:(alert?"10px":"")}}>{alert ? alertMsg : ''}</p>
                            <input ref={newLocalidade} style={{marginBottom:"5px"}} list="localidades" id="partida" type="text" placeholder="Desporto Favorito" required/>
                ...
                <div className={`popup ${popUpDeleteLocalidade ? 'active' : ''}`}>
                    <div className="overlay">
                        <div className="overlayContent">
                            <h1>Deseja eliminar este desporto?</h1>
                            <div className="butoesAcceptBack">
                ...
                <datalist id="localidades">
                    {allLocalidades.map((localidade) =>
                        <option key={localidade.idDesporto} value={localidade.nomeDesporto} />
                    )}
                </datalist>
        </>
    )
